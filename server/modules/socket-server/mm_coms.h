...
#define DLL_RAW_SAMPLES_PER_PKT 64
  typedef struct {
     uint  status;       /* Status of the MMIR III - reserved        */
     uchar log_att;      /* logical attenuation settings - see below */
     uchar time_stamp_lower;/* Time stamp value                      */
     uchar time_stamp_upper;/* Time stamp value                      */
      ...
      */
     /* First byte tx is left_emg followed by left_diff[0] */
     uchar left_emg;
     uchar left_diff[DLL_RAW_SAMPLES_PER_PKT]; 
     uchar right_emg;
     uchar right_diff[DLL_RAW_SAMPLES_PER_PKT];

  } Dllt_rt_raw;
#define DLL_RT_RAW_SIZE sizeof(Dllt_rt_raw)
